...
const selinuxdSocketTimeout = 5 * time.Second
...
func (r *ReconcileSP) reconcilePolicy(sp *spov1alpha1.SelinuxPolicy, l logr.Logger) (reconcile.Result, error) {
	...
	installed, err := r.isPolicyInstalled(sp)
	if err != nil {
...
func (r *ReconcileSP) isPolicyInstalled(sp *spov1alpha1.SelinuxPolicy) (bool, error) {
	...
	if err != nil {
		return false, errors.Wrap(err, "failed to create a request to selinuxd")
	}
	...
	return SliceContainsString(moduleList, GetPolicyName(sp.Name, sp.Namespace)), nil
}
